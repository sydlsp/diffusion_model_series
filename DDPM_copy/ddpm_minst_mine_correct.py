...
"""
上面已经实现了Unet的全部内容，Unet的主要作用是用来预测噪声的分布
"""
...
class GaussianDiffusion:
    ...
    def train_losses(self,model,x_start,t):  #gaussion_diffusion.train_losses(model,images,t)  #这里的images就是原始图像
        noise=torch.randn_like(x_start)

        x_noisy=self.q_sample(x_start,t,noise=noise)  #q_sample 是在第t步加噪完的结果
        predicted_noise=model(x_noisy,t)
        loss=F.mse_loss(noise,predicted_noise)  #就是这里存在一个小问题，就是为什么要向noise拟合
        return loss
